...
# Function to create a scatter plot with correlation and formatted x-axis labels
create_corr_plot <- function(x_var, df, point_color) {
  # Create a named vector for neat variable names
  pretty_names <- c(
    "GDP_growth_annual_" = "GDP Growth (annual %)",
    "Income_share_held_by_lowest_20" = "Income Share held by lowest 20%",
    "Control_of_Corruption_Estimate" = "Control of Corruption Estimate",
    "Life_expectancy_at_birth_total_years" = "Life expectancy at birth (years)",
    "Literacy_rate_adult_total__of_people_ages_15_and_above" = "Adult literacy rate (% ages 15+)",
    "Trade__of_GDP" = "Trade (% of GDP)",
    "Foreign_direct_investment_net_inflows_BoP_current_US" = "FDI Net inflows (current US$)",
    "Age_dependency_ratio__of_workingage_population" = "Age Dependency Ratio (% of working-age population)"
  )
  ...
  ggplot(df, aes_string(x = x_var, y = "Current_health_expenditure_per_capita_current_US")) +
    ...
    labs(
      title = paste("Correlation:", cor_value), 
      x = pretty_names[x_var], 
...
combined_plot <- wrap_plots(plots, ncol = 3) +
  plot_annotation(
    ...
    subtitle = "(The x-axis is labeled at the top of each plot, y-axis is labeled at the left side )",
    caption = "Source: World Development Indicators",
    theme = theme(
      plot.title = element_text(size = 19, face = "bold", hjust = 0.5),  # Center title
      plot.subtitle = element_text(size = 17, hjust = 0.5),              # Center subtitle
