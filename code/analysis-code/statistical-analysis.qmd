...
# 1. Create properly formatted results table
...
# 2. Create and save HTML table
unit_root_table %>%
  kbl(format = "html", digits = 4, align = "c",
      caption = "Panel Unit Root Test Results") %>%
  kable_styling(bootstrap_options = c("striped", "hover")) %>%
  add_footnote("Null hypothesis: All series contain unit roots", 
              notation = "none") %>%
...
# Option 1: Simple version with try()
adf_results <- by(train, train$CountryName, function(x) {
  try(adf.test(na.omit(x$Current_health_expenditure_per_capita_current_US)),
  silent = TRUE)
})

# Option 2: More robust version with error handling
