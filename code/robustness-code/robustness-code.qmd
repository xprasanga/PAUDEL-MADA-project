...
library(rsample)  # For analysis() and assessmen()
library(here)
library(ggplot2)
...
# Define a named vector for clean variable names
clean_names <- c(
  "(Intercept)" = "Intercept",
  "GDP_growth_annual_" = "GDP Growth (annual %)",
  "Income_share_held_by_lowest_20" = "Income Share (Lowest 20%)",
  "Control_of_Corruption_Estimate" = "Control of Corruption",
  "Life_expectancy_at_birth_total_years" = "Life Expectancy (years)",
  "Unemployment_youth_total__of_total_labor_force_ages_1524_modeled_ILO_estimate" = "Youth Unemployment (% of labor force, ages 15-24)",
  "Trade__of_GDP" = "Trade (% of GDP)",
  "Foreign_direct_investment_net_inflows_BoP_current_US" = "FDI Net Inflows (current US$)",
  "Access_to_electricity_rural__of_rural_population" = "Rural Electricity Access (% of rural population)",
  "Age_dependency_ratio__of_workingage_population" = "Age Dependency Ratio"
)

# Generate the table with formatted names
modelsummary(
  model,
  output = here("results", "tables", "testresult.png"),
  title = "",
  stars = c('*' = 0.1, '**' = 0.05, '***' = 0.01),
  coef_rename = clean_names,
  notes = list(
...
results <- data.frame(
  `Train Years` = paste(min(train_cv$Year), max(train_cv$Year), sep = "-"),
  `Test Year` = max(test_cv$Year),
  RMSE = rmse_val,
  stringsAsFactors = FALSE,
  check.names = FALSE  # Preserves the spaces in column names
)
  })
}
...
# First create a named vector for pretty variable names
neat_names <- c(
  "GDP_growth_annual_" = "GDP Growth",
  "Income_share_held_by_lowest_20" = "Income Share (Lowest 20%)",
  "Control_of_Corruption_Estimate" = "Control of Corruption",
  "Life_expectancy_at_birth_total_years" = "Life Expectancy",
  "Unemployment_youth_total__of_total_labor_force_ages_1524_modeled_ILO_estimate" = "Youth Unemployment",
  "Trade__of_GDP" = "Trade (% of GDP)",
  "Foreign_direct_investment_net_inflows_BoP_current_US" = "FDI Net Inflows",
  "Access_to_electricity_rural__of_rural_population" = "Rural Electricity Access",
  "Age_dependency_ratio__of_workingage_population" = "Age Dependency Ratio"
)

# Extract and prepare importance data with neat names
importance_df <- data.frame(
  Variable = names(rf_full_model$variable.importance),
  Importance = rf_full_model$variable.importance,
  stringsAsFactors = FALSE
) %>% 
  mutate(Variable = neat_names[Variable]) %>%  # Replace with neat names
  arrange(desc(Importance))

# Create importance plot with better formatting
importance_plot <- ggplot(importance_df, aes(x = reorder(Variable, Importance), 
                                           y = Importance)) +
  geom_col(fill = "steelblue", width = 0.7) +
  coord_flip() +
  labs(title = "Random Forest Variable Importance",
       x = NULL,  #  redundant axis title
...
ggsave(here("results", "figures", "rf_importance_plot.png"), 
       ...
       width = 10,  #  wider for better text fit
       height = 6, 
       dpi = 300)
```
...
```{r}
# Select and rename columns with clean names
rf_results_table <- rf_results %>%
  select(
    `Train Years` = Train_Years,
    `Test Year` = Test_Year,
    RMSE,
    `Predictors Used` = Predictors_Used,
    `mtry Used` = mtry_Used
  )

# Create and save the HTML table with formatted headers
rf_results_html <- kable(rf_results_table, 
                        format = "html",
                        digits = 2,
                        col.names = c("Train Years", "Test Year", "RMSE", 
                                     "No. of predictors", "mtry"),
                        caption = "") %>%
  kable_styling(bootstrap_options = c("striped", "hover"),
                full_width = FALSE,
                position = "center") %>%
  add_header_above(c(" " = 1, " " = 1,  # Empty spaces for first two columns
                    "Performance" = 1, 
                    "Model Parameters" = 2)) %>%
